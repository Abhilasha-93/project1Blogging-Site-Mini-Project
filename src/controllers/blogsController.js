...
const updateBlogs = async function (req, res) {
    try {
        ...
        if (decodedToken.authorId !== check.authorId.toString()) 
        return res.status(401).send({ status: false, msg: "User logged is not allowed to modify the requested blogs data" })
...
const delBlogs = async function (req, res) {
    try {
        ...
        if(check.isDeleted==true) return res.status(400).send({status:false,msg:"already deleted"})
...
const delBlogsByQuery = async function (req, res) {
    ...
    if (data.length==0) return res.status(411).send({ status: false, msg: "Data can not be empty" });
        ...
        if (auth === undefined) return res.status(401).send({status:false, msg: "not your blog ,you are not allowed to  delete this blog" })
